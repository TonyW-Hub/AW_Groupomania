    ...
    <Post v-for="post in posts" :key="post.id" >
        ...
        <div>
          <div class="col d-flex justify-content-between align-items-center p-0">
            <div>
              ...
              <span class="text-secondary font-weight-bold px-3 ml-2">{{ post.User.pseudo }}</span>
            </div>
            <span class="badge text-secondary align-self-start p-2">{{ post.createdAt.substr(0, 10).split("-").reverse().join("-") }}</span>
          </div>
          <div class="row mx-auto">
            <div class="col-11 card my-3 py-2 mx-auto post-content">
              {{ post.content }}
              <b-img-lazy v-if="post.imageUrl" :src="post.imageUrl" fluid-grow class="rounded mx-auto pt-1" alt="responsive image"></b-img-lazy>
            </div>
            <!-- <hr class="line w-100 mb-1 bg-secondary"> -->
          </div>
        </div>
      </template>
      ...
      <template v-slot:Comments v-if="comments !== null">
          <div v-for="(comment) in comments.filter((comment) => {return comment.postId == post.id })"
            ...
            <div v-if="comments.postId == posts.id" class="row d-flex justify-content-between align-items-center comment-area py-1 px-1 commentContent">
                <img :src="comment.User.imageUrl" class="imgComment align-self-center">
                <span id="userNameComment" class="align-self-center text-white px-1">{{ comment.User.pseudo }}</span>
                <p class="col-8 text-left mb-0 py-1 text-white commentContent">{{ comment.content }}</p>
...
export default {
  ...
  async created() {
    ...
    // GET ALL COMMENTS
    await axios
            .get('http://localhost:3000/api/posts/comment', {
              headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": 'Bearer ' + this.token
              }
  ...
  methods: {
    ...
    sendCom(id) {
      ...
      axios
        .post('http://localhost:3000/api/posts/' + id + '/comment', data, {
          headers: {
    ...
    deleteCom(id) {
      axios
        .delete('http://localhost:3000/api/posts/' + id + '/comment', {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": 'Bearer ' + this.token
          }
